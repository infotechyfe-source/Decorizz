...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    } else if (layout && ['Circle', 'Landscape'].includes(layout)) {
  ...
  const finalFilteredProducts = useMemo(() => {
    return filteredProducts.filter(p => {
      ...
      if (['Circle', 'Landscape'].includes(activeFilter)) {
        ...
        <div className="flex gap-8 items-start">
              ...
              <div className="overflow-y-auto flex-1 p-6 custom-scrollbar">
                ...
                <div className="mb-6 pb-6 border-b" style={{ borderColor: '#e5e7eb' }}>
                  <button
                    ...
                    className="flex items-center justify-between w-full mb-3 px-2 py-1 rounded-lg transition bg-teal "
                    style={{ fontWeight: 700, color: '#1f2937' }}
                  >
                    <h3 className='text-white'>Decor by Space</h3>
                    <ChevronDown
                  ...
                  {expandedSections.room && (
                    <div className="space-y-2">
                      {ROOM_OPTIONS.map(room => {
                        ...
                        return (
                          <label key={room.name} className="flex items-center justify-between cursor-pointer group">
                  ...
                  {expandedSections.categories && (
                    <div className="space-y-2">
                      {categoryNames.map(name => {
                        const isChecked = filters.categories.includes(name);
                        return (
          ...
          <div className="flex-1 w-full lg:order-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              {(['All', '2-Set', '3-Set', 'Square', 'Circle', 'Landscape'] as const).map(opt => (
                <button
                  key={opt}
